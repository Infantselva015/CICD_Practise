...
class SalesforceBranchAlignmentProcess extends AbstractProcess implements Serializable {
    ...
    private void writeConflictCsvFile(String releaseVersionTarget, String releaseVersionSource) {
        String header = "Resource,Owner,Vendor\n"
        StringBuilder conflicts = new StringBuilder(header)
        def conflictFiles = gitUtils.conflictFiles().split("\n")
        // Load info-users.json
        def userInfo = dsl.readJSON file: "devops/info-users.json"
        // Build a map of username to vendor
        def userVendorMap = [:]
        userInfo.each { entry ->
            entry.usernames.each { uname ->
                userVendorMap[uname.toLowerCase()] = entry.vendor
            }
        }
        conflictFiles.each { file ->
            // Get last two commits for the file
            def logOutput = dsl.sh(script: "git log -2 --pretty=format:'%h,%an,%ad' -- ${file}", returnStdout: true).trim().split('\n')
            dsl.echo("DEBUG: logOutput for ${file}: " + logOutput)
            def owner = ""
            // Find the last commit not by Jenkins
            for (entry in logOutput) {
                def parts = entry.split(',')
                if (parts.size() >= 3 && !parts[1].toLowerCase().contains('jenkins')) {
                    owner = parts[1]
                    break
                }
            }
            // Map owner to vendor using info-users.json
            def vendor = userVendorMap[owner.toLowerCase()] ?: ""
            conflicts.append("${file},${owner},${vendor}\n")
        }
        dsl.echo(conflicts.toString())
        dsl.dir("manual_procedures/${releaseVersionTarget}/${ALIGNMENT_FOLDER}") {
            dsl.writeFile file: "${releaseVersionSource}_#${dsl.BUILD_NUMBER}_Conflicts.csv", text: conflicts.toString()
        }
    }
    ...
    private Map schedulePullRequestAcceptanceJob() {
        ...
        if (!jobName.endsWith(JOB_CM_BRANCHALIGNMENT_SUFFIX)) {
            dsl.echo "WARNING: jobName '${jobName}' does not end with expected suffix '${JOB_CM_BRANCHALIGNMENT_SUFFIX}'. Pipeline will continue."
        }
    ...
    void printFinalJobResults() {
        ...
        cfg.addEntryToMap(MAP_KEY_EMAIL_MESSAGE, printResult, false)
