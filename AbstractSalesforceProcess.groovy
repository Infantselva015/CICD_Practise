...
abstract class AbstractSalesforceProcess extends AbstractProcess implements Serializable {
    ...
    void preDeploySalesforce() {
        ...
        //changes made for sf cli -Implementing SFCLI
        
        String sfConsumerKeyId
        String sfPrivateKeyId
        String sfUsernameId
         
        sfConsumerKeyId = "Salesforce_Jenkins_${targetEnv.toUpperCase()}_CONSUMER_KEY"
        sfPrivateKeyId = "Salesforce_Jenkins_${targetEnv.toUpperCase()}_PRIVATE_KEY"
        sfUsernameId = "Salesforce_Jenkins_${targetEnv.toUpperCase()}_USERNAME"

        cfg.addEntryToMap(MAP_KEY_SF_CONSUMER_KEY_ID, sfConsumerKeyId, true)
        cfg.addEntryToMap(MAP_KEY_SF_PRIVATE_KEY_ID, sfPrivateKeyId, true)
        cfg.addEntryToMap(MAP_KEY_SF_USERNAME_ID, sfUsernameId, true)
...
"""
    ...
    protected void doValidate(boolean doBackup) {
        ...
        String sfConsumerKeyId = cfg.getMapValue(MAP_KEY_SF_CONSUMER_KEY_ID)
        String sfPrivateKeyId = cfg.getMapValue(MAP_KEY_SF_PRIVATE_KEY_ID)
        String sfUsernameId = cfg.getMapValue(MAP_KEY_SF_USERNAME_ID)
        ...
        slfcUtils.validate(workingPath, slfcUrl, sfConsumerKeyId, sfPrivateKeyId, sfUsernameId, testLevel, testClsList, logFileSuffix, doBackup)
    }
    ...
    void quality() {
        ...
        try {
            //Added 
            boolean reportPMDOk = false
            String pmdReportFile = "pmdReport_${dsl.env.BUILD_NUMBER}.json"
            ...
            if (!skipSca && deltaClassesExists) {
                ...
                //Added
                if (dsl.fileExists("${workingPath}/${pmdReportFile}")) 
		{		
			reportPMDOk = true
			dsl.echo("PMD report file '${pmdReportFile}' found and will be included in validation report.")
			dsl.archiveArtifacts artifacts: "${pmdReportFile}", allowEmptyArchive: false
		} 
		else 
		{
			dsl.echo("Warning: PMD report file '${pmdReportFile}' not found. It will not be included.")
		}
            }
