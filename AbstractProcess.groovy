...
abstract class AbstractProcess {
    ...
    protected String getEmailMessageFirstPart(String buildResult) {
        String emailMessage = ""
        Configuration cfg = Configuration.getInstance()

        if (!"SUCCESS".equals(buildResult.toUpperCase())) {
            emailMessage = Utils.getNonSuccessStageResults(dsl.currentBuild) + "\n"
            if (cfg.getMapValue(MAP_KEY_UNSTABLE_MESSAGE) != null && cfg.getMapValue(MAP_KEY_UNSTABLE_MESSAGE)?.trim()) {
                emailMessage += "${cfg.getMapValue(MAP_KEY_UNSTABLE_MESSAGE)}\n"
            }
        }

        if (cfg.getMapValue(MAP_KEY_EMAIL_MESSAGE) != null && cfg.getMapValue(MAP_KEY_EMAIL_MESSAGE)?.trim()) {
            emailMessage += "${cfg.getMapValue(MAP_KEY_EMAIL_MESSAGE)}\n"
        }
    ...
    protected String getEmailMessageFinalPart() {
        String emailMessage = ""

        Configuration cfg = Configuration.getInstance()

        if (cfg.getMapValue(MAP_KEY_BLUE_OCEAN_URL) != null && cfg.getMapValue(MAP_KEY_BLUE_OCEAN_URL)?.trim()) {
            emailMessage = "Open Blue Ocean: ${cfg.getMapValue(MAP_KEY_BLUE_OCEAN_URL)}\n"
        }
    ...
    protected String getEmailMessage(String prefix, String buildResult) {
        String emailMessage = "${prefix} ${buildResult}\n"

        emailMessage += this.getEmailMessageFirstPart(buildResult)
        emailMessage += this.getEmailMessageFinalPart()
    ...
    String getSuccessEmailMessage() {
        return getEmailMessage("SUCCESS", "SUCCESS")
    }

    String getUnstableEmailMessage() {
        return getEmailMessage("WARNING", "UNSTABLE")
    }

    String getFailureEmailMessage() {
        return getEmailMessage("FAILURE", "FAILURE")
    }

    String getAbortEmailMessage() {
        return getEmailMessage("ABORTED", "ABORTED")
    }
