...
class SalesforceDeployProcess extends AbstractSalesforceProcess implements Serializable {
    ...
    void deploy() {
        ...
        if (isQuickDeploy) {
            ...
            slfcUtils.quickDeploy(workingPath, slfcUrl, sfConsumerKeyId, sfPrivateKeyId, sfUsernameId, quickDeployId)
        } else {
            slfcUtils.deploy(workingPath, slfcUrl, sfConsumerKeyId, sfPrivateKeyId, sfUsernameId, testLevel, testClsList)
        }
    }
    ...
    protected String getEmailMessageFirstPart(String buildResult) {
        String emailMessage = super.getEmailMessageFirstPart(buildResult)
        Configuration cfg = Configuration.getInstance()

        if ("SUCCESS".equals(buildResult.toUpperCase()) || "UNSTABLE".equals(buildResult.toUpperCase())) {
            if (cfg.getMapValue(MAP_KEY_NEXUS_ARTIFACT_URL) != null && cfg.getMapValue(MAP_KEY_NEXUS_ARTIFACT_URL)?.trim()) {
                emailMessage += "Download Nexus artifact: ${cfg.getMapValue(MAP_KEY_NEXUS_ARTIFACT_URL)}\n"
            }
        }
